...
def score_item(it: Dict[str, Any], f: Dict[str, Any]) -> float:
    ...
    featScore     = (sum(1 for k in feats if it.get("flags", {}).get(k)) / len(feats)) if feats else 0
...
def reasons(it: Dict[str, Any], f: Dict[str, Any]) -> List[str]:
    ...
    if f.get("budget_max") and it.get("price_yen") and it["price_yen"] <= f["budget_max"]:
        r.append(f"ご予算内（{it['price_yen']:,}円）")
    if f.get("walk_max") and it.get("station_walk_minutes") is not None and it["station_walk_minutes"] <= f["walk_max"]:
        r.append(f"駅徒歩{it['station_walk_minutes']}分")
    if f.get("pet_ok") and it.get("flags", {}).get("pet_ok"):
        r.append("ペット可")
    for k in f.get("must_have", []):
        if it.get("flags", {}).get(k): r.append(k)
    return r
...
def fallback_parse_query_to_filters(q: str) -> Dict[str, Any]:
    ...
    if m: f["budget_max"] = int(float(m.group(1)) * 100_000_000)
    m = re.search(r"(\d{3,5}(?:\.\d+)?)\s*万", qn)
    if m and "budget_max" not in f: f["budget_max"] = int(float(m.group(1)) * 10_000)
    m = re.search(r"徒歩\s*(\d{1,2})\s*分", qn)
    if m: f["walk_max"] = int(m.group(1))
    if "ペット" in q: f["pet_ok"] = True
    for n in [3,2,1]:
    ...
    if m:
        val = int([g for g in m.groups() if g][0]); f["min_area_sqm"] = val
    f["wards"] = [w for w in WARD_LIST if w in q]
    must = []
    if "南向き" in q:       must.append("south_facing")
    if "角部屋" in q:       must.append("corner")
    if "バルコニー" in q:   must.append("balcony")
    if "タワマン" in q or "タワーマンション" in q: must.append("tower_mansion")
...
def combined_text(doc: Dict[str, Any]) -> str:
    bits = [
        doc.get("name",""), doc.get("description",""), doc.get("address",""),
        doc.get("station_line") or "", doc.get("station_name") or "",
